...
function autocomplete(input, latInput, lngInput) {
  ...
  //if someone hits enter(= keycode 13) on the address field(to select a location from the google api dropdown), don't submit the form
...
function makeMap(mapDiv) {
  if (!mapDiv) return; //don't run the function unles page has a #map div
...
function typeAhead(search) {
  if (!search) return; //search element not found
  ...
  //handle keyboard inputs on the results
